...
function UpdateNotifications(state) {
	...
	for (let i = state.notifications.length - 1; i >= 0; --i) {
		...
		if (opening) {
			height_fraction = (time_now - notif.start_time) / kNotificationFadeInTime;
		} else if (closing) {
			height_fraction = (end_time - time_now) / kNotificationFadeOutTime;
		}
		
		let spacing_height = Math.ceil((kNotificationHeight + kSpacing) * height_fraction);
		if (spacing_height < 1) {
			continue;
		}
		let height = Math.max(0, spacing_height - kSpacing);
		let notif_html = '<div style="height: ' + spacing_height +
				'px; clear: both;"><button class="notification" style="background: ' +
				notif.bgcolor + '; height: ' + height + 'px;"'
		if (!opening && !closing) {
